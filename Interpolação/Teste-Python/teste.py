...
for i in range(out_size[0]):
    ...
    for j in range(out_size[1]):
        ...
        # Se não, significa que teremos que interpolar o valor para aquele pixel na imagem de saída
            ...
            # Verifica se foram encontrado apenas dois pixels de referência (variando apenas em y).
            # Se sim, aplica-se interpolação linear 
            if (x0 == x1):
                value = linear_interpolation(j/scale, y0, f00, y1, f01)    
            
            # Verifica se foram encontrado apenas dois pixels de referência (variando apenas em x).
            # Se sim, aplica-se interpolação linear 
            elif (y0 == y1):
                value = linear_interpolation(i/scale, x0, f00, x1, f10)
            
            # Caso contrário, temos um pixel intermédiario a quatro pixels da imagem de entrada.
            # Sendo assim, aplica-se a interpolação bilinear.
            else:
