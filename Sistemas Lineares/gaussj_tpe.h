/*
    Eliminação Gaussiana com pivoteamento completo considerando epsilon da máquina
*/
#include <limits>
#include <cmath>
...
template <class T> void gaussj(Matriz<T> &m){
    ...
    for (j=0; j<nc; j++){
        ...
        // Verificar se o valor (absoluto) a ser usado para escalar a linha i é menor que epsilon da máquina
        if (fabs(m[i][j]) <= std::numeric_limits<double>::epsilon()){
            std::cerr << "Pŕoximo da precisão da máquina" << std::endl;
            exit(0);
        }
    ...
    while (ipos_troca < nr*2 && pos_troca[ipos_troca] != 0){
        // Troca de linhas da parte aumentada da matriz - reverter a "troca de variáveis"
        for (l=0; l<nc; l++){
        ...
        // Troca das colunas da parte principal da matriz (coeficientes)
        for (l=0; l<nr; l++){
            aux = m[l][pos_troca[ipos_troca]-1];
            m[l][pos_troca[ipos_troca]-1] = m[l][pos_troca[ipos_troca+1]-1];
            m[l][pos_troca[ipos_troca+1]-1] = aux;
        }
        ipos_troca += 2;
    }
